...
namespace Automattic\WccomAiConnector\Models;
...
/**
 * Class for a WCCOM text generation model using OpenAI's Responses API.
 *
 * @since 1.0.0
 *
 ...
 * }
 */
class WccomTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    /**
     * {@inheritDoc}
     *
    ...
    /**
     * Gets request headers for the WCCOM proxy.
     *
     * The Playground blueprint writes the per-user WCCOM key into
     * `wccom_ai_api_key`, not the AI Client's provider credential store.
     *
     * @since 0.1.0
     *
    ...
    private function getRequestHeaders(): array
    {
        ...
        if (function_exists('get_option')) {
            $apiKey = (string) get_option('wccom_ai_api_key', '');
            if ($apiKey !== '') {
